...
class GeoDataset(InMemoryDataset):
    def __init__(self, root='../data', raw_name='ccs_dataset_modified_log.csv', processed_name='ccs_processed.pt', transform=None, pre_transform=None):
        self.filename = os.path.join(root, raw_name)
        
        self.df = pd.read_csv(self.filename)
        self.x = self.df[self.df.columns[1]].values
        self.y = self.df[self.df.columns[6]].values   
    ...
    def process(self):
        ...
        csv_file_path = '/home/vvd9fd/Documents/Bilodeau Group/Codes/0.Research/CCS_PyGeo/results/progress.csv'  
        ...
        for i, (x, y) in enumerate(zip(self.x, self.y), 1):
            ...
            if processed_samples % 1000 == 0:  # Registrar cada 100 iteraciones
